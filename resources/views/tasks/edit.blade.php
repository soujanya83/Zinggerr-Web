...
<script>
  $(document).ready(function() {
    ...
    $('#assignModal').on('show.bs.modal', function(event) {
        ...
        const taskId = button.data('task-id'); // Extract task ID from data-* attributes
    ...
    function fetchAssignmentData(id) {
        $.ajax({
            url: '{{ route("get.task.assignments", ":id") }}'.replace(':id', id),
            method: 'GET',
            success: function(response) {
                if (response.success) {
                    ...
                    // Check assigned users, roles, and courses
                    response.assignments.users.forEach(userId => {
                        $(`.user-checkbox[value="${userId}"]`).prop('checked', true);
                    });

                    response.assignments.roles.forEach(roleId => {
                        $(`.role-checkbox[value="${roleId}"]`).prop('checked', true);
                    });

                    response.assignments.courses.forEach(courseId => {
    ...
    $('.role-checkbox:checked, .course-checkbox:checked').each(function() {
        ...
        let itemHtml = `
            <div class="selected-item m-1 p-2 d-inline-flex align-items-center"
                ...
                style="background-color: #f0f0f0; border-radius: 30px; color:black">
        `;
