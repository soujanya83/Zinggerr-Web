                            ...
                            <div class="col-6">
                                <select id="filterRoleDropdown" class="form-select">
                                    ...
                                    @foreach ($roles as $roleData)
                                        <option value="{{ $roleData->display_name }}">{{ $roleData->display_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script>
  $(document).ready(function() {
    ...
    function fetchAssignmentData() {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    assignmentData.users = response.users.slice(0, 100); // Limit to 100 users
                    ...
                    updateUserListVisibility(); // Check initial visibility
                } else {
        ...
        assignmentData.courses.forEach(course => {
            $courseList.append(`
                <div class="course-item py-2 px-1" data-course-id="${course.id}">
                    <label class="d-flex align-items-center w-100 m-0">
                        <input type="checkbox" class="assign-checkbox course-checkbox me-2" value="${course.id}" data-name="${course.course_full_name}">
    ...
    // Build course-user mappings from assignedCourseUsers
    ...
    function applyInitialSelections() {
        ...
        updateUserSelectionsBasedOnRoles();
    }
    ...
    // Populate course users (aggregate users from selected courses)
    ...
    // Update user selections based on roles
    ...
    $('#assignModal').on('show.bs.modal', function() {
        ...
        $('.course-users-section').hide(); // Ensure user list is hidden on modal open
    });
...
<script>
    // Updated function to populate the role filter dropdown from the provided select element
    function populateRoleFilterDropdown() {
    // Get the values from the existing dropdown in the HTML
    const $existingDropdown = $('#filterRoleDropdown');
    const $roleFilterDropdown = $('.role-filter-dropdown');
    $roleFilterDropdown.empty();

    // Copy options from the existing dropdown
    $existingDropdown.find('option').each(function() {
        const value = $(this).val();
        const text = $(this).text();
        $roleFilterDropdown.append(`<option value="${value}">${text}</option>`);
    });
    }
