...
class PermissionsController extends Controller
{
    ...
    public function user_assign_permission($slug)
    {
        ...
        $assignedPermissions = PermissionRole::where('user_id', $user->id)
            ->pluck('permission_id')
            ->toArray();
    ...
    public function alluser_assign_permission($slug)
    {
        $user = User::where('slug', $slug)->firstOrFail();
        $allpermissions = Permission::all();
        $assignedPermissions = UsersPermission::where('user_id', $user->id)
            ->pluck('permission_id')
            ->toArray();

        return view('permissions.alluser_assign_permission', compact('user', 'allpermissions', 'assignedPermissions'));
    }
    ...
    //     $allPermissions=PermissionRole::where('user_id', $assignuser_id);
    //     $createdBy=$allPermissions->created_by;

    //     $roleBy=$allPermissions->role_id;
    ...
    public function user_assign_permissions(Request $request)
    {
        $assignuser_id = $request->user_id;
        $user_id = Auth::user()->id;
        $allPermissions = PermissionRole::where('user_id', $assignuser_id)->get();

        if ($allPermissions->isEmpty()) {
            return redirect()->route('userlist')->with('error', 'No permissions found for this user!');
        }
        $createdBy = $allPermissions->first()->created_by;
        $roleBy = $allPermissions->first()->role_id;
        $oldPermissions = $allPermissions->pluck('permission_id')->toArray();
        PermissionRole::where('user_id', $assignuser_id)->delete();
        ...
        if ($request->has('permissions')) {
            ...
            PermissionRole::insert($newPermissions);
        }
    ...
    public function alluser_assign_permissions(Request $request)
    {

        $assignuser_id = $request->user_id;

        UsersPermission::where('user_id', $assignuser_id)->delete();
        if ($request->has('permissions')) {
            $newPermissions = [];
            foreach ($request->permissions as $permission_id) {
                $uuid = (string) Guid::uuid4();
            ...
            UsersPermission::insert($newPermissions);
        }
